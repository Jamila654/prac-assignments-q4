...
# Set OPENAI_API_KEY and GEMINI_API_KEY from Streamlit secrets
try:
    os.environ["OPENAI_API_KEY"] = st.secrets["OPENAI_API_KEY"]
    gemini_api_key = st.secrets["GEMINI_API_KEY"]
except KeyError as e:
    st.error(f"Missing secret: {e}. Please set OPENAI_API_KEY and GEMINI_API_KEY in .streamlit/secrets.toml")
    st.stop()
...
agent = Agent(
    ...
    For unrelated input (e.g., "What's the weather?"), return {"error": "I'm your Reminder Agent, here to set reminders!"}.
    Return valid JSON wrapped in ```json\n...\n``` with keys: message, remind_at, error (if applicable).
    Example input: "set reminder for June 2, 2025, at 5:25 PM for meeting t"
    Example output: ```json\n{"message": "meeting", "remind_at": "2025-06-02T17:25:00"}\n```
    Unrelated input: "What's the weather?"
    Unrelated output: ```json\n{"error": "I'm your Reminder Agent, here to set reminders!"}\n```
    """,
...
async def parse_input(user_input):
    try:
        ...
        return {"error": f"I'm your Reminder Agent, here to set reminders! Error: {str(e)}"}
...
def main():
    ...
    with st.form("reminder_form"):
        user_input = st.text_input("Set a Reminder", help="E.g., set reminder for June 2, 2025, at 11:30 PM for meeting")
        if st.form_submit_button("Submit"):
            try:
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                ...
                st.error(f"Error processing reminder: {str(e)}")
                return
            if "error" in data:
                st.error(data["error"])
            else:
